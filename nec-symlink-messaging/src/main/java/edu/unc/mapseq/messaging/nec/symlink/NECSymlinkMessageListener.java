package edu.unc.mapseq.messaging.nec.symlink;
...
public class NECSymlinkMessageListener implements MessageListener {

    private final Logger logger = LoggerFactory.getLogger(NECSymlinkMessageListener.class);

    private WorkflowBeanService workflowBeanService;

    public NECSymlinkMessageListener() {
        super();
    }
            ...
            for (int i = 0; i < entityArray.length(); ++i) {
                ...
                if (entityJSONObject.has("entity_type")) {
                    ...
                    if ("Workflow run".equals(entityType) || WorkflowRun.class.getSimpleName().equals(entityType)) {
                        workflowRun = EntityUtil.getWorkflowRun(workflowBeanService.getMaPSeqDAOBean(), "NECSymlink",
